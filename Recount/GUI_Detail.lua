...
do
	...
	local function say(msg, channelType, channel)
		if channelType == "self" then
			-- To self.
			print(msg)
		else
			-- To channel.
			SendChatMessage(msg, channelType, nil, channel)
		end
	end
	...
	local function onUpdate(this)
		if not queuedMessages or #queuedMessages==0 then
		...
		local msg, channelType, channel = string.split("\t", queuedMessages[1])
		say(msg, channelType, channel)
		table.remove(queuedMessages,1)
	end

	function Recount:QueueSendChat(msg, channelType, channel)
		channelType = channelType or "say"
		channel = channel or ""
		table.insert(queuedMessages, string.format("%s\t%s\t%s", msg, channelType, channel))
...
function me:ReportElement(loc,loc2)
	if Recount.DetailWindow.SummaryMode.DamageMode then
		Recount:QueueSendChat(L["Recount"].." - "..L["Outgoing"].." "..self.Title:GetText().." "..L["Damage Report for"].." "..Recount.DetailWindow.Showing,loc,loc2)
		--SendChatMessage(L["Recount"].." - "..L["Outgoing"].." "..self.Title:GetText().." "..L["Damage Report for"].." "..Recount.DetailWindow.Showing,loc,nil,loc2)
	else
		Recount:QueueSendChat(L["Recount"].." - "..L["Incoming"].." "..self.Title:GetText().." "..L["Damage Report for"].." "..Recount.DetailWindow.Showing,loc,loc2)
	...
	for _, HitType in pairs(SummaryHitTypes) do 
		...
		if Num~="-" then
			Recount:QueueSendChat(HitType..": "..Num.."x "..Per,loc,loc2)
...
function me:ReportSummarySet(loc,loc2)
	Recount:QueueSendChat(L["Recount"].." - "..self.Title:GetText().." "..L["Report for"].." "..Recount.DetailWindow.Showing,loc,loc2)
	...
	for _,v in pairs(SummarySet) do
		if self[v] then
			Recount:QueueSendChat(self[v]:GetText(),loc,loc2)
...
function Recount:ReportDetail(amount,loc,loc2)
	...
	if Recount.DetailWindow.PieMode:IsVisible() then
		...
		if not Recount.DetailWindow.Locked then
			...
			Recount:QueueSendChat("Recount - "..Recount.DetailWindow.TitleText,loc,loc2)
			--SendChatMessage("Recount - "..Recount.DetailWindow.TitleText,loc,nil,loc2)
			for i=1,amount do
				Entry=UpperTable[i]
				Recount:QueueSendChat(i..". "..(Entry[1] or "").." "..(Entry[6] or "").." "..(Entry[2] or "").." ("..math.floor(Entry[4]+0.5).."%)",loc,loc2)
			...
			Recount:QueueSendChat("Recount - "..Recount.DetailWindow.TitleText..": "..UpperTable[Recount.DetailWindow.PieMode.Selected][1],loc,loc2)
			...
			for i=1,amount do
				...
				Recount:QueueSendChat(Text,loc,loc2)
	...
	elseif Recount.DetailWindow.DeathMode:IsVisible() then
		if Recount.DetailWindow.DeathMode.SelectedNum and Recount.DetailWindow.DeathMode.Data[Recount.DetailWindow.DeathMode.SelectedNum] then
			...
			Recount:QueueSendChat("Recount - "..L["Death for "]..Recount.DetailWindow.DeathMode.WhosDeaths..L[" by "]..(DeathData.KilledBy or L["Unknown"])..L[" at "]..date("%H:%M:%S",DeathData.DeathAt),loc,loc2)
			...
			for i=Start,Entries do
				if MsgTimes[i]<0 then
					Recount:QueueSendChat(string.format("%.2f - ("..L["Health"]..": %s) "..Msgs[i],MsgTimes[i],Health[i]),loc,loc2)
