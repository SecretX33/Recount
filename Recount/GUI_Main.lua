...
function Recount:ReportData(amount,loc,loc2)
	...
	if type(dataMode[6])=="function" then
		Recount:QueueSendChat("Recount - "..dataMode[6](),loc,loc2)
		--SendChatMessage("Recount - "..dataMode[6](),loc,nil,loc2)
	else
		if ConvertDataSet[Recount.db.profile.CurDataSet] then
			Recount:QueueSendChat("Recount - "..dataMode[1]..L[" for "]..ConvertDataSet[Recount.db.profile.CurDataSet],loc,loc2)
		...
		elseif Recount.FightName then -- Elsia: Cover nil error here.
			Recount:QueueSendChat("Recount - "..dataMode[1]..L[" for "]..Recount.FightName,loc,loc2)
	...
	if not MainWindow_Settings.HideTotalBar and Total > 0 then
		...
		Recount:QueueSendChat("0. Total  "..(math_floor(10*Total)/10).." ("..PerSec..(math_floor(1000)/10).."%)",loc,loc2)
	...
	for i=1,amount do
		if reportTable[i] and reportTable[i][2]>0 then
			...
			Recount:QueueSendChat(i..". "..reportTable[i][1].."  "..(math_floor(10*reportTable[i][2])/10).." ("..PerSec..(math_floor(1000*reportTable[i][2]/Total)/10).."%)",loc,loc2)
